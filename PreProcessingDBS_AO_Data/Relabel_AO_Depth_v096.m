function Relabel_AO_Depth_v096(driveLOC , year , driveLetter)
...
if driveLOC
    AOLoc = [driveLetter,'\S3_AO_MatlabData_S3\' , num2str(year)];
else
    AOLoc = [driveLetter,'\S3_AO_MatlabData_S3'];
end
...
for fdir = 1:length(foldernamesFinal)
    ...
    if contains(testfile,'Set') && isempty(dirDateFiles)
        ...
        for dai = 1:length(diractual)
            ...
            [~, ~, ~] = checkAndpack(dateLoc, dai, diractual, neuroOmFlag, driveLOC , driveLetter);
...
function [newLoc, neuroOmFlag] = rename_file(depthFiles, actFileDir, doneTag, driveLOC , driveLetter)
% RENAME_FILE
...
oldLoc = pwd;
if driveLOC
    newLoc = strcat(driveLetter,'\S4_AO_ProcMatlabData_S4\',actFileDir); % NOT sure if should be S3 or S4?
    if ~exist(newLoc,'dir')
        mkdir(newLoc)
    end
end
...
function [newLoc, lfpBool, toSaveFiles] = RenameCheckLFP(dateLoc, dai, diractual, driveLOC , driveLetter)
% RENAMECHECKLFP
...
if strcmp(regexp(testFileA,'Trgt','match'),'Trgt')
    doneTag = 1;
    [newLoc, neuroOmFlag] = rename_file(depthFiles, actFileDir, doneTag, driveLOC , driveLetter);
else
    doneTag = 0;
    [newLoc, neuroOmFlag] = rename_file(depthFiles, actFileDir, doneTag , driveLOC, driveLetter);
...
function [preProLoc, lfpBool, toSaveFiles] = checkAndpack(dateLoc, dai, diractual, neuroOmFlag, driveLOC , driveLetter)

[preProLoc, lfpBool, toSaveFiles] = RenameCheckLFP(dateLoc, dai, diractual, driveLOC , driveLetter);
