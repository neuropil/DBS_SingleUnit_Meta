...
if exist('W:\','dir')
    if driveLOC
        AOLoc = ['W:\S3_AO_MatlabData_S3\' , num2str(year)];
    else
        AOLoc = 'W:\S3_AO_MatlabData_S3';
    end
    ...
    warndlg('Check for W:\Data'); % NEED to Obtain access to Drive
end
...
for fdir = 1:length(foldernamesFinal)
    ...
    if contains(testfile,'Set') && isempty(dirDateFiles)
        ...
        for dai = 1:length(diractual)
            ...
            [~, ~, ~] = checkAndpack(dateLoc, dai, diractual, neuroOmFlag, driveLOC);
...
function [newLoc, neuroOmFlag] = rename_file(depthFiles, actFileDir, doneTag, driveLOC)
% RENAME_FILE
...
oldLoc = pwd;
if driveLOC
    newLoc = strcat('W:\S4_AO_ProcMatlabData_S4\',actFileDir); % NOT sure if should be S3 or S4?
    if ~exist(newLoc,'dir')
        mkdir(newLoc)
    end
end
...
function [newLoc, lfpBool, toSaveFiles] = RenameCheckLFP(dateLoc, dai, diractual, driveLOC)
% RENAMECHECKLFP
...
if driveLOC
    actFileDir = strrep(tempdateLoc, 'W:\S3_AO_MatlabData_S3\', '');
    preProLoc = ['W:\S4_AO_ProcMatlabData_S4\' , actFileDir];
else
...
if strcmp(regexp(testFileA,'Trgt','match'),'Trgt')
    doneTag = 1;
    [newLoc, neuroOmFlag] = rename_file(depthFiles, actFileDir, doneTag, driveLOC);
else
    doneTag = 0;
    [newLoc, neuroOmFlag] = rename_file(depthFiles, actFileDir, doneTag , driveLOC);
...
function [preProLoc, lfpBool, toSaveFiles] = checkAndpack(dateLoc, dai, diractual, neuroOmFlag, driveLOC)

[preProLoc, lfpBool, toSaveFiles] = RenameCheckLFP(dateLoc, dai, diractual, driveLOC);
