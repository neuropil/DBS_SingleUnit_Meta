function Relabel_AO_Depth_vt(inPS)
...
arguments
    ...
    inPS.newLOC (1,1) logical = 1; % IN USE for CASE NUMBER
...
if inPS.driveLOC
    ...
    AOLoc = 'C:\Users\Admin\Desktop\testa';
else
    AOLoc = uigetdir();
end
...
if inPS.newLOC
    ...
    NewLoc = 'C:\Users\Admin\Desktop\testb';
else
    NewLoc = uigetdir();
end
...
for fdir = 1:length(foldernamesFinal)
    ...
    if contains(testfile,'Set') && isempty(dirDateFiles)
        ...
        for dai = 1:length(diractual)
            ...
            [~, ~, ~] = checkAndpack(dateLoc, dai, diractual, neuroOmFlag, 1 , inPS.driveLetter , NewLoc , AOLoc);
...
function [newLoc, neuroOmFlag] = rename_file(depthFiles, actFileDir, doneTag, driveLOC , ~ , NewLoc)
% RENAME_FILE
...
oldLoc = pwd;
if driveLOC
    newLoc = strcat(NewLoc, actFileDir); % NOT sure if should be S3 or S4?
    if ~exist(newLoc,'dir')
        mkdir(newLoc)
    end
end
    ...
    if ~ismember('ttlDONE.txt',ttlCcheck)
        ...
        for ttlfi = 1:length(depthFiles)
            ...
            if neuroOmFlag
                ...
                ttlCheck = contains(tTLNames,'C1_DI00');
                ttlLogInd = find(ttlCheck);
            end
        ...
        if combineFlag
            ...
            for exC = 1:length(allNames)
                ...
                if tempExType == 1
                    ...
                    if contains(tempExName,'Time') % if == 1 then has to do with Time
                        % Check if Begin or End
                        if contains(tempExName,'Begin')
...
function [newLoc, lfpBool, toSaveFiles] = RenameCheckLFP(dateLoc, dai, diractual, driveLOC , driveLetter , NewLoc , AOLoc)
% RENAMECHECKLFP
...
actFileDir = strrep(tempdateLoc, AOLoc, '');
preProLoc = [NewLoc, actFileDir];
postProLoc = [AOLoc ,actFileDir];
...
if strcmp(regexp(testFileA,'Trgt','match'),'Trgt')
    doneTag = 1;
    [newLoc, neuroOmFlag] = rename_file(depthFiles, actFileDir, doneTag, driveLOC , driveLetter, NewLoc);
else
    doneTag = 0;
    [newLoc, neuroOmFlag] = rename_file(depthFiles, actFileDir, doneTag , driveLOC, driveLetter , NewLoc);
...
fNums = cellfun(@(x) x{2}, depN1, 'UniformOutput',false);
fNzeros = cellfun(@(x) strcmp(x(3),'0') , fNums);
...
while di < length(depNums)
    ...
    if sum(tInd) == 1
        ...
        for dii = 1:sum(tInd) - 1
            ...
            %%% CHANGED 01-16-2020
            zCount = fNzeros(di + tC);
            
            if zCount
                reIndex{di + tC} = [teReInd(1:length(teReInd)-4),'000',num2str(ti),'.mat'];
            else
                reIndex{di + tC} = [teReInd(1:length(teReInd)-4),'00',num2str(ti),'.mat'];
            end
            tC = tC + 1;
            ti = ti + 1;
        end
        ...
        for rnI = 1:length(reNameExtAdd)
            ...
            elseif length(oldName) > 16
                ...
                if strcmp(oldName(5),'-')
                    ...
                    zeroCheck1 = strcmp(tempOD{3}(end - 1),'0');
                    zeroCheck2 = strcmp(tempOD{3}(3),'0');
                    zeroCheck3 = strcmp(tempOD{3}(end),'0');
                    tempDL = tempOD{2};
                    if length(tempDL) == 5
                        tempNumNa = oldName(5:9);
                        numNaParts = strsplit(tempNumNa,'.');
                        
                        if zeroCheck1 && zeroCheck2
                            newName = strcat('0',[numNaParts{:}],'000',oldName(length(oldName)),'.mat');
                        elseif zeroCheck1 && ~zeroCheck2
                            newName = strcat('0',[numNaParts{:}],'000',oldName(length(oldName)),'.mat');
                        elseif zeroCheck3
                            newName = strcat('0',[numNaParts{:}],'00',oldName(length(oldName)-1),'0','.mat');
                        else
                            newName = strcat('0',[numNaParts{:}],'00',oldName(length(oldName)-1:length(oldName)),'.mat');
                        end
                    else
                        tempNumNa = oldName(5:10);
                        numNaParts = strsplit(tempNumNa,'.');
                        if zeroCheck1
                            ...
                            newName = strcat([numNaParts{:}],'00',oldName(length(oldName)),'.mat');
                        end
                    end
                end
                
                if zeroCheck1 && ~zeroCheck2
                    oldName = oldName(1:length(oldName)-3);
                else
...
function [preProLoc, lfpBool, toSaveFiles] = checkAndpack(dateLoc, dai, diractual, neuroOmFlag, driveLOC , driveLetter , NewLoc , AOLoc)

[preProLoc, lfpBool, toSaveFiles] = RenameCheckLFP(dateLoc, dai, diractual, driveLOC , driveLetter , NewLoc , AOLoc);
